...
begin
  ...
  v_level := case
    when v_level in ('high_school', 'senior') then 'senior_high'
    when v_level in ('junior', 'middle_school') then 'junior_high'
    else v_level
  end;
  v_subject := coalesce(
  ...
  v_source_kind := case
    when coalesce(v_payload->>'source_kind', new.payload->>'source_kind', '') in ('curriculum_content', 'learning_performance')
      then coalesce(v_payload->>'source_kind', new.payload->>'source_kind')
    when coalesce(v_payload->>'indicator_type', new.payload->>'indicator_type', '') like '%學習內容%' then 'curriculum_content'
...
comment on function private.sync_t0_review_decision_to_gate() is
  'Mirrors label T0 review decisions into T0 staging and normalizes level aliases such as high_school -> senior_high.';
